...
export default function LabelTechnologyManager() {
    ...
    const fetchItems = async () => {
        ...
        try {
            ...
            console.error("データ取得の失敗の失敗:", error)
        } finally {
            setIsLoading(false)
        }
    }
        ...
        return (
            <div className="space-y-3 mt-4">
                {items.map((item) => (
                    <div
                        key={item.id}
                        className={`flex items-center gap-2 p-3 rounded-md border ${selectedIds.includes(item.id) ? "bg-muted border-primary" : ""
                            }`}
                    >
                        <Checkbox
