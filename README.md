...
- **記事共有のための Markdown 編集体験**  
  Article ページでは、サイト上で Markdown を記入して記事を書けるようにしました。難しかったのは、編集中に画像を挿入したい場合でも、ローカル画像はそのままではパスが通らず使えない点です。そこで、挿入ボタンから追加した画像は一度だけデータベースに保存して仮 URL を発行し、Markdown 上で即座に参照できるようにしました。記事を保存する際にその画像を正式なデータとして保存し、保存されなかった場合はそのまま破棄することで、自然な執筆体験と不要データの抑制を両立しています。
